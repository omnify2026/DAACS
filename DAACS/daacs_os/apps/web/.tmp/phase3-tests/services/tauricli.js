...
exports.CLI_PROVIDER_STORAGE_KEY = exports.CLI_WORKSPACE_STORAGE_KEY = void 0;
exports.getSavedWorkspacePath = getSavedWorkspacePath;
exports.setSavedWorkspacePath = setSavedWorkspacePath;
exports.openWorkspaceDialog = openWorkspaceDialog;
exports.isTauri = isTauri;
exports.getAgentsMetadataJson = getAgentsMetadataJson;
exports.getSavedCliProvider = getSavedCliProvider;
exports.setSavedCliProvider = setSavedCliProvider;
exports.runCliCommand = runCliCommand;
exports.getCliWhich = getCliWhich;
exports.getWorkspacePath = getWorkspacePath;
exports.saveLocalOfficeState = saveLocalOfficeState;
exports.loadLocalOfficeState = loadLocalOfficeState;
exports.clearLocalOfficeState = clearLocalOfficeState;
exports.prewarmOmniCli = prewarmOmniCli;
exports.getAgentPrompt = getAgentPrompt;
exports.getSkillPromptForRole = getSkillPromptForRole;
exports.getSkillPromptForCustom = getSkillPromptForCustom;
exports.getSkillBundleSummary = getSkillBundleSummary;
exports.getSkillCatalog = getSkillCatalog;
exports.getAvailableSkillIds = getAvailableSkillIds;
exports.getPromptingSequencerSystemPrompt = getPromptingSequencerSystemPrompt;
exports.getPromptingSequencerChannelId = getPromptingSequencerChannelId;
exports.savePromptingSequencerTodo = savePromptingSequencerTodo;
exports.loadPromptingSequencerTodo = loadPromptingSequencerTodo;
exports.markPromptingSequencerItemDone = markPromptingSequencerItemDone;
exports.runPmCliCommand = runPmCliCommand;
exports.runFrontendCliCommand = runFrontendCliCommand;
exports.runBackendCliCommand = runBackendCliCommand;
exports.parsePmTaskLists = parsePmTaskLists;
const constants_1 = require("../constants");
exports.CLI_WORKSPACE_STORAGE_KEY = constants_1.STORAGE_KEY_CLI_WORKSPACE;
exports.CLI_PROVIDER_STORAGE_KEY = constants_1.STORAGE_KEY_CLI_PROVIDER;
function getSavedWorkspacePath() {
    if (typeof localStorage === "undefined")
        return null;
    try {
        const v = localStorage.getItem(exports.CLI_WORKSPACE_STORAGE_KEY)?.trim();
...
function setSavedWorkspacePath(InPath) {
    try {
        if (InPath == null || InPath.trim() === "")
            localStorage.removeItem(exports.CLI_WORKSPACE_STORAGE_KEY);
        else
            localStorage.setItem(exports.CLI_WORKSPACE_STORAGE_KEY, InPath.trim());
    }
    catch {
        /**/
    }
...
function getSavedCliProvider() {
    if (typeof localStorage === "undefined")
        return null;
    try {
        const v = localStorage.getItem(exports.CLI_PROVIDER_STORAGE_KEY)?.toLowerCase();
        if (v === "gemini" || v === "codex")
            return v;
        return null;
    }
...
function setSavedCliProvider(provider) {
    try {
        localStorage.setItem(exports.CLI_PROVIDER_STORAGE_KEY, provider);
    }
    catch {
        /**/
    }
}
async function runCliCommand(instruction, options) {
...
async function getWorkspacePath(InProjectId) {
    if (!isTauri())
        return null;
    try {
...
async function saveLocalOfficeState(projectId, snapshot) {
    if (!isTauri() || !window.__TAURI__?.core)
        return false;
    try {
...
async function loadLocalOfficeState(projectId) {
    if (!isTauri() || !window.__TAURI__?.core)
        return null;
    try {
...
async function clearLocalOfficeState(projectId) {
    if (!isTauri() || !window.__TAURI__?.core)
        return false;
    try {
...
async function getPromptingSequencerSystemPrompt(InProjectName, InChannelId) {
    if (!isTauri() || !window.__TAURI__?.core)
        return "";
    try {
...
function getPromptingSequencerChannelId(InRoleKey) {
    const key = (InRoleKey ?? "").trim().toLowerCase();
    if (!key)
        return HashFnv1aHex("unknown");
    return HashFnv1aHex(key);
}
async function savePromptingSequencerTodo(InProjectName, InTodo) {
    if (!isTauri() || !window.__TAURI__?.core)
        return false;
    try {
...
async function loadPromptingSequencerTodo(InProjectName, InChannelId) {
    if (!isTauri() || !window.__TAURI__?.core)
        return null;
    try {
...
async function markPromptingSequencerItemDone(InProjectName, InChannelId, InNumber) {
    if (!isTauri() || !window.__TAURI__?.core)
        return null;
    try {
...
async function runPmCliCommand(InInstruction, InOptions) {
...
async function runFrontendCliCommand(InInstruction, InOptions) {
...
async function runBackendCliCommand(InInstruction, InOptions) {
    let systemPrompt = null;
...
async function parsePmTaskLists(InStdout) {
    if (isTauri() && window.__TAURI__?.core) {
        try {
