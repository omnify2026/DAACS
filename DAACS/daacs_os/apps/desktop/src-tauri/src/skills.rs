...
const DESKTOP_RESOURCES_SKILLS_BUNDLE: &str = "Resources/skills/agent_bundles.yaml";
const DESKTOP_RESOURCES_SKILLS_REPO: &str = "Resources/skills/repository";
const BUNDLE_PREFIX: &str = "bundle_";

fn desktop_pack_root() -> Option<PathBuf> {
...
pub fn resolve_skill_roots() -> Vec<PathBuf> {
    ...
    if let Some(desktop) = desktop_pack_root() {
        let bundled_repo = desktop.join(DESKTOP_RESOURCES_SKILLS_REPO);
        if bundled_repo.is_dir() {
            candidates.push(bundled_repo);
        }
    }
...
pub fn resolve_bundles_config() -> PathBuf {
    ...
    if let Some(desktop) = desktop_pack_root() {
        let bundled = desktop.join(DESKTOP_RESOURCES_SKILLS_BUNDLE);
        if bundled.is_file() {
            return bundled;
        }
    }
